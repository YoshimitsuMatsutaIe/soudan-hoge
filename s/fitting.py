"""
フィッティングできました．
curve_fitはパラメータ初期値の設定がかなりシビアなようです．
適当な値（例えば(0,0)など）を突っ込んでも収束しません．


'ルンゲクッタ法で連立方程式を解く際にパラメータを1つ固定して解くことができず、'
<- classを実装してクラス変数にするかグローバル変数を使えば良いと思います．

'また実験値(t(時刻),I(感染者))の内tが方程式に直接表れていないためです'
<- tまで数値積分すれば良いという意味でした．
"""
